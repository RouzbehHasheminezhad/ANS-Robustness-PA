...
def get_effect_of_n():
    import pandas as pd
    effect_of_n = {"": ["$\\beta=" + str(x) + "$" for x in [0.05, 0.1, 0.2, 0.25, 0.3, 0.4]], 1000: [], 10000: [],
                   20000: []}
...
def get_effect_of_k():
    import pandas as pd
    effect_of_k = {"": ["$\\beta=" + "{:.2f}".format(x) + "$" for x in [0.05, 0.1, 0.2, 0.25, 0.3, 0.4]], 3: [], 4: [], 5: []}
...
def generate_table_2():
    ...
    with open("results/tables/table_2.txt", "w") as f:
        f.write(res)
...
def generate_table_1():
    ...
    df = pd.DataFrame({"$\\beta=" + str(x) + "$": ["{:.3f}".format(get_cluster_score(10000, 3, x))] for x in [0.05, 0.1, 0.2, 0.25, 0.3, 0.4]})
    body = df.to_latex(index=False, escape=False, column_format="|c|c|c|c|c|c|c|").replace("\\toprule\n", "\cline{1-7}\n\multirow{2}{*}{$n=10000, k=3$} &").replace("\midrule\n", "\cline{2-7}\n& ").replace("\\bottomrule", "\hline")
    res = header + "\caption{" + caption + "}" + body + label + footer
    with open("results/tables/table_1.txt", "w") as f:
        f.write(res)
